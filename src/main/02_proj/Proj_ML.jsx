...
const modelpy = [
  {
    id: 1,
    image: act1,
    name: "Naive Bayes Phishing Detection Model using Python",
    description: (
      <>
        <strong>Naive Bayes Machine Learning Model:</strong> This project is a
        machine learning model that uses the Naive Bayes algorithm to detect
        phishing websites. It was developed using Python and various libraries
        such as scikit-learn and pandas. The model was trained on the dataset
        from Kaggle, and using the preprocessing steps, it was able to achieve
        an accuracy of 97.5% on the test set from 10,000 total records.
        Moreover, the model also provides a user-friendly interface for users to
        input a URL and receive a prediction on whether the website is phishing
        or not.
      </>
    ),
  },
...
export default function MachineLearning({ sectionRef, onCountChange }) {
  const dark = useTheme();
  const t = tk(dark);

  useEffect(() => {
    onCountChange?.(modelpy.length);
  }, []);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <FaPython style={{ fontSize: "1.3rem", color: t.heading }} />
